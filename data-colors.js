...
// 例外。ここに並ぶのは「通説では駄目」あるいは「この索引の原則を破る」組です。
// それでも成立するのには条件があり、条件を書かずに例外だけ真似ると外します。
// breaks が空文字なら、破っているのは通説のほうです。
// 診断（judge）がこれらを落とすのは正しい動作です。落ちる理由が分かっていて
// 選ぶのが例外で、知らずに選ぶのは失敗です。
const EXCEPTIONS = [
  { id: 'x1', name: '黒に茶',         colors: ['black', 'brown', 'black'],      breaks: '',
    myth: '黒と茶は合わない',
    why: '茶が十分に暗ければ、黒との間に残るのは明度差ではなく色みの差だけです。明度が近いほど、二色は同じ面として読まれます。',
    cond: '茶は明度30以下。黒は光沢のないものに限ります。エナメルの黒と茶は、明度が離れて喧嘩します。' },

  { id: 'x2', name: '紺に黒',         colors: ['navy', 'black', 'charcoal'],    breaks: '',
    myth: '紺と黒は喧嘩する',
    why: '喧嘩するのは、境目が曖昧なときだけです。革やニットのように段差のある素材で切ると、二色は混ざらずに並びます。',
    cond: '境目に素材の段差を作ること。同じ素材で紺と黒を続けると、色を間違えたように見えます。' },

  { id: 'x3', name: '深緑にボルドー', colors: ['green', 'bordeaux', 'brown'],   breaks: '02',
    myth: '',
    why: '色みのある色が3つあるので、原則02 は落とします。それでも揃うのは、3色とも同じだけ濁って同じだけ暗いからです。原則03 のほうは、むしろ満たしています。',
    cond: '3色の明度差を15以内に収めること。どれか一つでも鮮やかだと、その一つだけが浮きます。' },

  { id: 'x4', name: 'カーキにピンク', colors: ['pink', 'khaki', 'brown'],       breaks: '02',
    myth: '',
    why: 'これも色数で落ちます。ただし3色とも暖色で、ピンクをくすませればカーキと同じトーンの位置に落ちます。「土の色に近い」という一点で共通しているためです。',
    cond: 'ピンクは彩度を落としたものだけ。鮮やかなピンクだと、カーキが軍の色に戻って対立します。' },

  { id: 'x5', name: '灰にベージュ',   colors: ['gray', 'beige', 'brown'],       breaks: '',
    myth: '冷たい灰と温かいベージュは混ぜない',
  ...
  { id: 'x7', name: '紺にオレンジ',   colors: ['navy', 'navy', 'orange'],       breaks: '',
    myth: '補色は喧嘩する',
    why: '補色をそのまま同じ面積で並べれば喧嘩します。面積を靴紐ほどまで絞ると、対立ではなく合図になります。山の道具がこの配色を使うのは、見つけてもらうためです。原則05 のとおり、足元に置けば通ります。',
    cond: 'オレンジは全体の5%未満。靴・靴下・タグのどれか一点だけです。上半身に持つと途端に破綻します。' },

  { id: 'x8', name: '紫に茶',         colors: ['purple', 'brown', 'brown'],     breaks: '',
    myth: '紫は何にも合わない',
    why: '紫の青みを、茶の赤みが受けます。色相環では離れていますが、どちらも「赤を含む暗い色」で明度もほぼ同じなので、間にグレーを挟まなくても繋がります。',
    cond: '紫は明度30以下。明るい紫だと、茶が汚れて見えます。' },
];
